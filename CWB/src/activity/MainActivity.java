...
public class MainActivity extends Activity {
	...
	Button btn1, btnHome;
	String count = "20";
	String uri = "https://api.weibo.com/2/statuses/public_timeline.json?access_token=" + CApp.ACCESS_TOKEN + "&" + "count=" + count;
	String JSON, previous_cursor, next_cursor, hasvisible, total_number;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		btnHome.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				startActivity(new Intent(getApplication(), WeiboHomeActivity.class));
			}
		});
	...
	public class PublicWeiboListViewAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup group) {
			...
			StatusBean statusBean = list.get(list.size() - 1 - position);
	...
	void readNewPublicWeibos1() {
		
	}

	void readNewPublicWeibos() {
		new Thread(new Runnable() {

			@Override
			public void run() {
				try {
					HttpResponse response = HttpUtil.getResponseByGET(uri);
					if (response.getStatusLine().getStatusCode() == 200) {
